...
using LegendaryLogic;

namespace LegendaryLogic.Replays
{
    public class ReplayRecorder
    {
        ...
        public ReplayRecorder(string Executing, string Server, int GameId, string Region, string Key)
        {
            ExecutingDirectory = Executing;
            try
            {
                this.GameId = GameId;
                this.Region = Region;
                this.Server = "http://" + Server;
                Directory.CreateDirectory(Path.Combine(ExecutingDirectory, "cabinet", GameId + "-" + Region));

                File.WriteAllText(Path.Combine(ExecutingDirectory, "cabinet", GameId + "-" + Region, "key"), Key);
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    Path.Combine(ExecutingDirectory, "cabinet", GameId + "-" + Region, "version"));
                    ...
                    using (StreamWriter outfile = new StreamWriter(Path.Combine(ExecutingDirectory, "cabinet", GameId + "-" + Region, "token")))
                    {
                        outfile.Write(token);
                    }
                        ...
                        using (WebClient client = new WebClient())
                        {
                            for (int i = 1; i < LastChunk + 1; i++)
                            {
                                client.DownloadFile(
                                    ...
                                    GameId,
                                    i),
                                    Path.Combine(ExecutingDirectory, "cabinet", GameId + "-" + Region, "chunk-" + i));
            ...
            using (WebClient client = new WebClient())
            {
                ...
                if (LastChunkNumber == ChunkId)
                {
                    ...
                    client.DownloadFile(
                            ...
                            GameId,
                            1),
                        Path.Combine(ExecutingDirectory, "cabinet", GameId + "-" + Region, "chunk-" + 1));
                    ...
                    client.DownloadFile(
                        ...
                        Region,
                        GameId),
                    Path.Combine(ExecutingDirectory, "cabinet", GameId + "-" + Region, "endOfGameStats"));
                ...
                if (ChunkId % 2 == 0)
                {
                    ...
                    if (KeyFrameId != 0)
                    {
                        client.DownloadFile(
                            ...
                            GameId,
                            KeyFrameId),
                        Path.Combine(ExecutingDirectory, "cabinet", GameId + "-" + Region, "key-" + KeyFrameId));
                    }
                }
                ...
                client.DownloadFile(
                        ...
                        GameId,
                        ChunkId),
                    Path.Combine(ExecutingDirectory, "cabinet", GameId + "-" + Region, "chunk-" + ChunkId));
